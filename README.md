...
5. **Done.** Primary preview view: real content rendering — line-number gutter, per-line syntax-highlighted and wrapped rows via `internal/preview`'s `Load`/`BuildDisplayRows` — plus Up/Down/PageUp/PageDown scrolling and `g` goto-line, both clamped per §2.1 and cached/restored per `openfiles.Entry` so switching between open files preserves each one's scroll position exactly. The wrap width is currently the full terminal width minus the gutter (no split/popup pane-width cap yet — that's stage 6); the empty state renders its hint message and ignores scroll/goto-line keys as required.
6. Layout & rendering: header content per overlay, tree explorer's dual split/popup, badge rendering, selection highlight polish.
