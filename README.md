...
- `internal/tree/` — the lazily-loaded node model, flattening, navigation semantics, and the refresh/merge logic behind live updates (spec §2, §6, §9).
- `internal/ignore/` — the dependency-free `.gitignore`/`.dirtreeignore` pattern subset (spec §3).
- `internal/index/` — the background full-tree index used by jump mode, including live rebuilds (spec §5, §6).
- `internal/match/` — the shared substring/glob query-matching rule (spec §10).
- `internal/preview/` — file reading, best-effort syntax highlighting, and line wrapping for the preview pane (spec §7).
- `internal/layout/` — the pure split-view/popup layout math (spec §11).
- `internal/spinner/` — the delayed-loading-indicator timing/frame logic (spec §12).
- `internal/watch/` — the `fsnotify`-backed, debounced filesystem-change watcher driving live refresh (spec §6); OS-facility-adjacent like `internal/ui`, verified manually.
