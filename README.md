...
A file whose leading bytes contain a NUL byte opens as a read-only **hex view** (`preview.TierBinary`, SPEC.md §2.1a) instead of failing to open: an offset gutter, a hex-byte grid, and an ASCII column, reading only the bytes needed for the current viewport (`preview.ReadRange`) rather than holding the file resident regardless of size. Goto-offset (`g`, accepting hex or decimal input) and a byte/ASCII-substring find (`/`, always a background scan via `internal/hexfind`, since a hex view never holds a file's full content resident) mirror goto-line and in-file find's own UX; copy mode does not apply to a hex view. Manually verified in tmux against a 256KB synthetic fixture (`examples/data/hexfile`, `make -C examples hexfile`): rendering, goto-offset (including EOF clamping), find-and-jump, and Home/End/Page Up/Page Down navigation.
