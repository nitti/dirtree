...
Every opened file also gets a background pass (`internal/preview.StreamIndex`, stages 1-2 of the streaming-preview design proposed in `docs/STREAMING_PREVIEW_DESIGN.md`) that gates goto-line and drives a file-legend "building…" spinner while running, and splits files into two tiers by size (`preview.HighlightCeiling`, default 25MB): a file at or under the ceiling is fully read/highlighted in the background then kept resident, same as before, just non-blocking; a file over the ceiling is never fully resident — it renders as plain text and is read on demand in small windows, seeking through the background pass's line-offset index, so opening an arbitrarily large file no longer costs memory proportional to the file's size. In-file find is not yet implemented for the over-the-ceiling tier (a later addition).
