...
The pure-logic layers (`internal/tree`, `internal/ignore`, `internal/index`, `internal/match`, `internal/preview`, `internal/openfiles`, `internal/layout`, `internal/spinner`) have automated test coverage. `internal/ui` is hand-verified in a real terminal (tmux) as of stage 3 for what's implemented so far (tree navigation, expand/collapse, Space/`a` open actions, `e` to reopen the explorer, jump mode from both entry points including reveal-in-tree and open-into-list, quit); the open-files overlay and real preview content are not yet reachable.
