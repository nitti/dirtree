...
Every opened file also gets a background line-offset index (`internal/preview.StreamIndex`) that gates goto-line and drives a file-legend "building…" spinner while it's running — stage 1 of the streaming-preview design proposed in `docs/STREAMING_PREVIEW_DESIGN.md`; reading/highlighting/wrapping themselves are still capped exactly as before, ahead of that design's later Tier A/B split.
