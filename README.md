...
5. **Done.** Primary preview view: real content rendering — line-number gutter, per-line syntax-highlighted and wrapped rows via `internal/preview`'s `Load`/`BuildDisplayRows` — plus Up/Down/PageUp/PageDown scrolling and `g` goto-line, both clamped per §2.1 and cached/restored per `openfiles.Entry` so switching between open files preserves each one's scroll position exactly.
6. **Done.** Tree explorer's dual split/popup layout (§5.1), recomputed every frame off `internal/layout`'s `ComputeTreePaneWidth`/`ShouldSplitView`: split view puts the tree pane on the left (sized to the longest visible label, clamped, growing into leftover width once the preview pane hits its 120-column cap) with a vertical rule and the primary preview view — still visible, read-only — on the right; popup view floats a centered, bordered window (titled with the root path, with the tree's own footer hint) over the unmodified, full-width primary preview view. Live resize flips between them.
...
The pure-logic layers (`internal/tree`, `internal/ignore`, `internal/index`, `internal/match`, `internal/preview`, `internal/openfiles`, `internal/layout`, `internal/spinner`) have automated test coverage. `internal/ui` is hand-verified in a real terminal (tmux) as of stage 6 for what's implemented so far: tree navigation, expand/collapse, Space/`a` open actions, `e` to reopen the explorer, jump mode from both entry points, the open-files overlay including Shift-Up/Shift-Down reordering, real preview rendering with scrolling and goto-line, per-entry scroll-state restoration across switches, the split-view/popup layout including a live resize flip between them, and quit.
