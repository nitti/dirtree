...
- Because terminals deliver no key-up event, a release is inferred from a gap in `q`'s key-repeat stream: a gap longer than the release-gap threshold resets an unconfirmed hold, a gap still under it leaves the hold running, and the check is a no-op when no hold is in progress at all (`TestCheckQuitHoldReleaseResetsAfterGap`, `TestCheckQuitHoldReleaseKeepsHoldWithinGap`, `TestCheckQuitHoldReleaseNoopWhenNotHolding`, `internal/ui/app_test.go`).
- The release-gap threshold itself tightens once a steady repeat cadence is established: with only one `q` event seen so far it's the generous initial threshold (tolerating a slow OS initial-repeat delay), but once a second `q` event has arrived it drops to a much tighter steady threshold, so a release partway through an active hold is detected — and the header reverts — within a frame or two rather than after the initial threshold's full window (`TestCheckQuitHoldReleaseUsesTightGapOnceRepeating`, `TestCheckQuitHoldReleaseKeepsHoldWithinTightGapOnceRepeating`, `internal/ui/app_test.go`).
- Once a hold is confirmed (held the full duration), quitting itself is deferred to a subsequently-detected release rather than happening the instant the duration elapses: a confirmed hold whose `q` repeat stream goes quiet actually quits, using the tight steady-cadence release-gap (a confirmed hold has necessarily already seen several repeats); a confirmed hold with `q` still actively repeating does not quit yet, however long it's already been confirmed for (`TestCheckQuitHoldReleaseQuitsOnceConfirmedAndReleased`, `TestCheckQuitHoldReleaseDoesNotQuitConfirmedHoldStillRepeating`, `internal/ui/app_test.go`). This deferral exists so `q` key-repeat events still physically in flight when the duration elapses are drained by the app's own event loop instead of leaking into the shell once the process exits and the terminal returns to normal input mode.
...
- The hold-to-quit gesture (§5.2), confirmed against real OS/terminal key-repeat behavior rather than synthetic timing: a quick tap of `q` does not quit and the header/title bar's attention-grabbing variant, if it appears at all, reverts to normal almost immediately; physically holding `q` down shows the header switch color, count down via the left-to-right fade, and quit the app once held for roughly a second, with the row fully faded by the moment it quits; releasing partway through the fade aborts it and the header snaps back to its normal content within a frame or two — not several hundred milliseconds later — rather than continuing to fade or quitting late; releasing right around when the hold completes (rather than well after) does not leave any stray `q` characters typed into the shell prompt once the app exits.
