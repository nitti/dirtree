...
- The bottom-right badge shows the completion message immediately after indexing finishes, and stops showing anything once its display+fade window has fully elapsed.
- Debug-only always-show mode is a build-time switch (`-tags spinnerdebug`), not something exercised by the normal automated test suite; manual verification is to build once without the tag and confirm the badge behaves normally (threshold-suppressed, then completion message + fade), then build with `-tags spinnerdebug` on a small directory and confirm: the spinner appears immediately, stays visible for at least its configured minimum duration (2 seconds in the prototype) even though real indexing finishes almost instantly, then hands off to the completion message and fade exactly as the non-debug path does; jump mode's indexing-blocked behavior is unaffected either way.
