...
- Narrow-terminal legend tiering, resized live in a real terminal (§5.2): shrinking the primary preview view's header progressively drops the root path, then `[o] quick open`/`[s] search` (priority 2), while `[b] browse`, `[tab] open files`, and `[q] quit` (priority 1) remain visible down to a very narrow width; confirmed similarly for at least one other view (e.g. content search's header dropping Ctrl+U/regex/expand-collapse before Return/Escape) — no entry is ever seen truncated mid-text, and entries never visibly reorder as the terminal is resized narrower and then wider again.
