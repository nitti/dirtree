...
- Content search persistence (§9.2): typing a query, then pressing Escape, then reopening with `s` (from either entry point) shows the exact same query, results, selection, mode, and per-file disclosure state as before Escape was pressed — nothing is reset just by leaving and returning to the overlay; Ctrl+U, backspacing the query to empty, or typing a new query all reset the result set (and its disclosure state back to expanded-by-default).
- Content search's regex mode (§9.1, §9.2): Ctrl+R toggles it on/off (header title switches to "search (regex)" and back) and immediately rescans the current query under the new mode; a regex query (e.g. `foo\d+`) matches lines a literal substring search wouldn't, and the same query switched back to substring mode is matched completely literally instead; an invalid pattern (e.g. an unclosed group) shows an inline "invalid regex" message in the results area instead of a scan or a crash, and correcting the pattern clears the error and resumes scanning.
