...
- Minimum terminal size (§6.4): resizing a live session below 40 columns or below 10 rows (in a multiplexer pane, or a real terminal window) replaces whatever was on screen — primary preview view or any overlay — with a too-small screen showing the current and required dimensions, confirmed in both the too-narrow and too-short directions independently as well as both at once; resizing back above both thresholds resumes the exact previous view (e.g. an overlay that was open is still open, a file's scroll position is unchanged) rather than resetting anything. Also confirm the too-small screen itself doesn't crash or garble when the terminal is resized far below its own minimum (e.g. down to a handful of columns/rows), since it can't rely on the width/height it's itself reporting as insufficient.
