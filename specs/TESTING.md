...
- Quick open's header shows a bold, all-caps "QUICK OPEN" mode label and legend correctly showing Return-to-open and Ctrl+U-to-clear, while the query lives on its own row directly beneath the header (not sharing a row with the legend), rendered with a background distinct from the match list below it — the same input-row convention content search uses (§9.2).
