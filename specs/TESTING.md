...
- Quick open's directory coloring (§4.2): a directory row in the match list renders in a color distinct from file rows, confirmed by querying a substring that matches both a directory and files (e.g. a common path segment) and seeing the directory row visually set apart; the selected row's highlight still applies (and takes precedence) whether the selection lands on a file or a directory row.
