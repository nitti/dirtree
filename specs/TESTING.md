...
- Slash-to-expand: typing `/` when the current match set is exactly one directory expands that directory, resets the query to empty, narrows the jump scope to that directory's descendants, and moves the browser selection to it; covered by `TestJumpMatchesScopeNarrowsToSubdirectory` in `internal/tree` for the underlying scoped-matching primitive.
- Slash-to-expand can be repeated to drill down multiple levels in one jump-mode session (match, `/`, match again under the newly-expanded directory, `/`, and so on).
...
- Quick open's header shows a bold, all-caps "QUICK OPEN" mode label and legend correctly showing Return-to-open, Tab/Shift-Tab and Page Up/Down to move the selection, and Ctrl+U-to-clear, while the query lives on its own row directly beneath the header (not sharing a row with the legend), rendered with a background distinct from the match list below it — the same input-row convention content search uses (§9.2).
