...
- The file title bar, directly above the preview content, shows the open-files list's total entry count ahead of the displayed entry's root-relative path (not its absolute path) whenever a file is open, in both the primary preview view and underneath the open-files-list overlay, and disappears when no file is displayed; the count reads `1 file` (singular) with exactly one entry open and `N files` otherwise, and updates immediately as files are opened or closed elsewhere in the app (`TestDrawFileTitleBarShowsOpenFileCount`, `internal/ui/views/preview_test.go`).
